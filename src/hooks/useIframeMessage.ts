import { RefObject, useCallback, useEffect, useState, Dispatch, SetStateAction, useRef, useMemo } from 'react'
...
export type MessageHandler<Message> = (message: Message, dispatch: MessageDispatcher<Message>) => void
...
function decodeMessage<Message>(raw: any): [Message, string | undefined] | undefined {
  try {
...
function isPrivateMessage(message: any): message is PrivateMessage {
  return (
    Object.prototype.hasOwnProperty.call(message, '__private') && '__private' in message && message.__private === true
  )
}

export function useIframeMessage<Message = any>(
  ...
  const eventListener = useCallback(
    (event: MessageEvent) => {
      const [message, fromId] = decodeMessage<Message | PrivateMessage>(event.data) || []
      if (message) {
  ...
  useEffect(() => {
    if (!isParent) {
      ...
      dispatchPrivate({ type: 'mounted', id, __private: true })
      setChildId(id)
    }
...
export function useIframeEvent<EventName = string>(
  ...
  const [dispatch] = useIframeMessage(handleMessage, options)
...
export function useIframeSharedState<S>(
  ...
  const handler: MessageHandler<SharedStateMessageType<S>> = useCallback((message) => {
    switch (message.type) {
  ...
  const [dispatch] = useIframeMessage(handler, options)
...
function useEventListener(handler: MessageEventListener, el?: HTMLElement | Window) {
  ...
  useEffect(() => {
    ...
    const eventListener = (event: Event) => savedHandler.current?.(event as MessageEvent)
